...
<div class="row">
	<div class="col-md-3">
		<div class="box box-solid">
			<div class="box-header with-border">
				<div class="pull-right box-tools">
	                <a class="btn btn-primary btn-sm pull-right" id="add_type">新增快递</a>
	             </div>
				<h4 class="box-title">快递类型</h4>
			</div>
			<div class="box-body">
		...
		<div class="box box-solid">
			<div class="box-header with-border">
				<div class="pull-right box-tools">
	                <a class="btn btn-primary btn-sm pull-right" id="add_business">新增商家</a>
	             </div>
	                ...
	                <div class="row">
	                    ...
	                    <div class="col-sm-6">
	                        <div id="example1_filter" class="dataTables_filter pull-right">
	                            <div class="btn-group">
	                                @pcan($path .'.create')
	                                <button class="btn btn-default" id="add_express"><i class="fa fa-truck"></i></button>
	                                @endpcan
...
var express = tables.data;
var express_type = <?php echo $express_type->toJson() ?>;
var express_business = <?php echo $express_business->toJson() ?>;
...
if(express_type.length > 0){
	for(var i in express_type){
		addType(express_type[i]);
	}
}

if(express_business.length > 0){
	for(var i in express_business){
		addBusiness(express_business[i]);
	}
}

if(express.length > 0){
	for(var i in express){
		addExpress(express[i]);
	}
}
...
function sidebarEvent(data , type){
	...
	var title = type ? '快递':'商家';
	var url = type ? '{{ route('data.logistics.expresstype.index') }}':'{{ route('data.logistics.expressbusiness.index') }}';
	obj.data(data);
...
$('#add_express').click(function(event) {
	var data = {};
	data.host = "http://";
...
$('#add_type').click(function(event) {
	...
	swal({
		...
		Rbac.ajax.request({
	        href: '{{ route('data.logistics.expresstype.store') }}',
	        data: $('form[name="swal-form"]').serialize(),
	        successFnc:function(r){
	        	addType(r.datas);
	        }
...
$('#add_business').click(function(event) {
	var data = {title:'商家名称'};
	swal({
		title: '新增商家',
		html: formatTemplate(data,$('script[name="sidebar-form"]').html()),
		...
		Rbac.ajax.request({
	        href: '{{ route('data.logistics.expressbusiness.store') }}',
	        data: $('form[name="swal-form"]').serialize(),
	        successFnc:function(r){
	        	addBusiness(r.datas);
	        }
